...
class Trash(FallingObjects):
    ...
    def __init__(self, app, full_screen, player):
        ...
        self._backgrounds = sorted([x for x in utils.get_backgrounds("Trash")])
        ...
        self._targets = [Target(desired_width=self._x_screen * Target.percentage, img_path=target_path,
                                target_type=target_type) for (target_type, target_path) in utils.get_targets("Trash")]

        self._projectiles = []
        for i, (projectile_type, projectile_path) in enumerate(utils.get_projectiles("Trash")):
